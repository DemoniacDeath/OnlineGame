...
class Client {
  constructor(canvas, status, network) {
    ...
    this.entity_id = network.socketId;
  ...
  processInputs() {
    if (!this.entities[this.entity_id]) return;
...
socket.on('connect', () => {
  var client = new Client(element("client_canvas"), element("client_status"), new Network(socket));
